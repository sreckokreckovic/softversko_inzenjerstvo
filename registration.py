...
class Registration(c.CTk):
    def __init__(self,db,master=None):
        ...
        self.password = self.create_elements(self.frame, "Password", 5,show='*')
        self.register_b = c.CTkButton(self.frame, text='Registruj se',command = self.register,width=200,font=('Verdana',15))
        ...
        self.login_b = c.CTkButton(self.button_frame, text="Imate već nalog? Prijavite se", font=('Verdana',15),width=200,command=self.open_login_window)
        self.login_b.grid(row=0, column=0, padx=10) 
        self.all_books = c.CTkButton(self.button_frame, text="Pogledajte knjige", font=('Verdana',15),width=220)
    ...
    def register(self):
        ...
        if not name and not surname and not email and not age and not password:
            tk.messagebox.showerror(title='Greska', message='Morate popuniti sva polja')
            return

        try:
            self.db.register_user(name, surname, age, contact, email, password, is_admin=0)
            tk.messagebox.showinfo(title='Obavjestenje', message='Uspjesno registrovan korisnik')
            self.clear_all()
        except Exception as e:
            tk.messagebox.showerror(title='Greska', message='Vec postoji korisnik sa istom email adresom')
