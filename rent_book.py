...
class RentBook(c.CTk):
    def __init__(self, db, master=None):
        super().__init__(master)
        self.db = db
        self.master = master

        self.title("Iznajmi Knjigu")
        self.geometry("1080x720")
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(0, weight=1)

        self.title_label = c.CTkLabel(
            self, text="Iznajmi knjigu", font=("Verdana", 20))
        ...
        self.delete_book_b = c.CTkButton(
            self.frame, text='Iznajmi knjigu', command=self.rent_book, width=200, font=('Verdana', 15))
    ...
    def rent_book(self):
        ...
        try:
            self.db.add_reservation(book_id, user_id)
            tk.messagebox.showinfo(
                    title='Rezervacija', message='Uspjesno rezervisano')
